...
public class UploadController {
	...
	public void formUploadPage() {}		// void 별도의 정보를 return 하지 않는다.
	//classpath:/template/formUpload.thml
	
	@PostMapping("uploadForm")
	public String formUploadFile
			(@RequestPart MultipartFile[] images) {	// 업로드 파일 여러개 안하면 [] 빼도됨
//		log.info(images[0].getOriginalFilename());
	...
	public List<String> uploadFile(@RequestPart MultipartFile[] uploadFiles) {
	    ...
	    for(MultipartFile uploadFile : uploadFiles){
	        ...
	        //UUID 고유 식별자를 의미함 (다른 사용자들의 파일이름 등 중첩이 안되게)
	...
	private String makeFolder() {
		String str = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy/MM/dd"));	//오늘날짜로 자동생성
		// LocalDate를 문자열로 포멧
